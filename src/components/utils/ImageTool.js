        ...
        exifRotateAngle(file).then(file=>{
            ...
            reader.onload = ()=>{
                ...
                img.onload = ()=>{
                    ...
                    if(file.exifdata && file.exifdata.angle !== 0){
...
let imgCompressAndOrientationAdjust2File = (file,compressConfig)=>{
    return new Promise((resolve,reject)=>{
        imgCompressAndOrientationAdjust(file,compressConfig).then(base64=>{
            resolve(blob2File(base64ToBlob(base64,file.type),file))
        })
    })
}
export const ImageTool = {
    getImgWidthAndHeight:getImgWidthAndHeight,
    imgCompressAndOrientationAdjust2File:imgCompressAndOrientationAdjust2File,
